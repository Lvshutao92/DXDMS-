...
//    NSLog(@"---78787878-----%@",arrData);
    ...
    [Manager requestPOSTWithURLStr:KURLNSString(@"common/address") paramDic:nil token:nil finish:^(id responseObject) {
        ...
        NSArray *arrData = (NSArray *)diction;
...
//    [viewHeader setBackgroundColor:kHeaderViewColor];
    viewHeader.backgroundColor = RGBACOLOR(39, 194, 183, 1.0);
    ...
    if (self.rowOfProvince < self.allProvinces.count) {
        ...
        if (self.rowOfCity < province.city.count) {
            ...
            if (self.rowOfTown < city.town.count) {
                ...
                arr = @[province.name,city.name,town.name,province.code1,city.code2,town.coder];
            }
        }
    }
    return arr;
