//
//  ChanPinManagerTableViewController.m
//  DXDMS
//
...
@interface ChanPinManagerTableViewController ()
@property(nonatomic,strong)NSMutableArray *arr;
@end

@implementation ChanPinManagerTableViewController
...
- (void)viewDidLoad {
    ...
    self.arr = [@[@"产品列表",@"类型列表",@"系列列表"]mutableCopy];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ImgCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        if ([cell.labText.text isEqualToString:@"产品列表"]) {
            ProductViewController *jituan = [[ProductViewController alloc]init];
            UINavigationController *navi = [[UINavigationController alloc]initWithRootViewController:jituan];
            jituan.navigationItem.title = @"产品列表";
            [self presentViewController:navi animated:YES completion:nil];
        }
        if ([cell.labText.text isEqualToString:@"系列列表"]) {
            XilieViewController *jituan = [[XilieViewController alloc]init];
            jituan.navigationItem.title = @"系列列表";
            UINavigationController *navi = [[UINavigationController alloc]initWithRootViewController:jituan];
            [self presentViewController:navi animated:YES completion:nil];
        }
        if ([cell.labText.text isEqualToString:@"类型列表"]) {
            LeiXingViewController *jituan = [[LeiXingViewController alloc]init];
            jituan.navigationItem.title = @"类型列表";
            UINavigationController *navi = [[UINavigationController alloc]initWithRootViewController:jituan];
            [self presentViewController:navi animated:YES completion:nil];
        }
}
