...
@interface LeiXingViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
{
    NSInteger page;
    NSInteger totalnum;
...
- (void)viewDidLoad {
    ...
    [self setUpReflash];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(leixing:) name:@"leixing" object:nil];
}
- (void)leixing:(NSNotification *)text{
    [self setUpReflash];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.lab1.text = [NSString stringWithFormat:@"系列名称：%@",model.typeName];
    return cell;
}
...
- (void)clickadd{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"新增" message:@"类型名称" preferredStyle:UIAlertControllerStyleAlert];
    [alert addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = @"类型名称";
    }];
    [alert addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
    [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        UITextField *text1 = [alert.textFields firstObject];
        //NSLog(@"%@",text1.text);
        NSDictionary *dic = @{@"typeName":text1.text};
        [Manager requestPOSTWithURLStr:KURLNSString(@"product/producttype/add") paramDic:dic token:nil finish:^(id responseObject) {
...
- (void)clickeditbtn:(UIButton *)sender{
    ...
    __weak typeof (self) weakSelf = self;
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"编辑" message:@"类型名称" preferredStyle:UIAlertControllerStyleAlert];
    [alert addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.text = model.typeName;
    }];
    [alert addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
    [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        UITextField *text1 = [alert.textFields firstObject];
        //NSLog(@"%@",text1.text);
        NSDictionary *dic = @{@"typeName":text1.text};
        NSString *str = [NSString stringWithFormat:@"%@/%@",KURLNSString(@"product/producttype/update"),model.id];
...
- (void)clickdelebtn:(UIButton *)sender{
    ...
    NSIndexPath *indexpath = [self.tableview indexPathForCell:cell];
    ModelOne *model = [self.dataArray objectAtIndex:indexpath.row];
    NSString *urlStr = [NSString stringWithFormat:@"%@/%@",KURLNSString(@"product/producttype/delete"),model.id];
    __weak typeof (self) weakSelf = self;
...
- (void)loddeList{
    ...
    __weak typeof(self) weakSelf = self;
    NSDictionary *dic = @{@"likeTypeName":textf.text};
    NSString *str = [NSString stringWithFormat:@"%@?currentPage=1&pageSize=10&sortName=id&sortType=desc",KURLNSString(@"product/producttype/page")];
    [Manager requestPOSTWithURLStr:str paramDic:dic token:nil finish:^(id responseObject) {
        NSDictionary *diction = [Manager returndictiondata:responseObject];
...
- (void)loddeSLList{
    ...
    __weak typeof(self) weakSelf = self;
    NSDictionary *dic = @{@"likeTypeName":textf.text};
    NSString *str = [NSString stringWithFormat:@"%@?currentPage=%ld&pageSize=10&sortName=id&sortType=desc",KURLNSString(@"product/producttype/page"),page];
    [Manager requestPOSTWithURLStr:str paramDic:dic token:nil finish:^(id responseObject) {
        NSDictionary *diction = [Manager returndictiondata:responseObject];
        if ([Manager judgeWhetherIsEmptyAnyObject:[diction objectForKey:@"rows"]] == YES) {
            NSMutableArray *arr = [diction objectForKey:@"rows"];
