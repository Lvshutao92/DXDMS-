...
@interface FPXX_ViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
{
    NSInteger page;
    NSInteger totalnum;
    UIView *bgSearchView;
...
@implementation FPXX_ViewController
...
- (void)viewDidLoad {
    ...
    [self setUpReflash];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(fkzh:) name:@"fpxx" object:nil];
}
...
- (void)lodStatus{
    __weak typeof (self) weakSelf = self;
    [Manager requestPOSTWithURLStr:KURLNSString(@"partner/partnerinvoice/inoviceTypeEnums") paramDic:nil token:nil finish:^(id responseObject) {
        NSDictionary *diction = [Manager returndictiondata:responseObject];
        //NSLog(@"-------%@",diction);
        NSMutableArray *arr = (NSMutableArray *)diction;
        for (NSDictionary *dicc in arr) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    LRViewBorderRadius(cell.editbtn, 7, 1, [UIColor redColor]);
    LRViewBorderRadius(cell.delebtn, 7, 1, [UIColor redColor]);
    cell.editbtn.hidden = YES;
    cell.lab7.hidden = NO;
    cell.lab8.hidden = NO;
    [cell.delebtn setTitle:@"编辑" forState:UIControlStateNormal];
    ...
    cell.lab2.text = [NSString stringWithFormat:@"注册地址：%@",model.invoiceAddr];
    cell.lab3.text = [NSString stringWithFormat:@"开户银行：%@",model.invoiceBankName];
    cell.lab4.text = [NSString stringWithFormat:@"银行账号：%@",model.invoiceBankNo];
    cell.lab5.text = [NSString stringWithFormat:@"注册电话：%@",model.invoicePhone];
    cell.lab6.text = [NSString stringWithFormat:@"纳税人识别码：%@",model.invoiceCode];

    cell.lab7.text = [NSString stringWithFormat:@"发票抬头：%@",model.invoiceTitle];
    for (ModelOne *statusModel in self.statusArray) {
        if ([statusModel.key isEqualToString:model.invoiceType]) {
            cell.lab8.text = [NSString stringWithFormat:@"发票类型：%@",statusModel.value];
        }
    }
    
    
    [cell.delebtn addTarget:self action:@selector(clickeditbtn:) forControlEvents:UIControlEventTouchUpInside];
    return cell;
}
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if ([tableView isEqual:self.tableview1]) {
        ...
        if (self.dataArray1 == self.statusArray) {
            ...
            text3.text = [self.aarr2 componentsJoinedByString:@" "];
            [self.tableview1 reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
        }
    }
}
...
- (void)clickeditbtn:(UIButton *)sender{
    ...
    FPXX_edit_ViewController *add = [[FPXX_edit_ViewController alloc]init];
...
- (void)loddeList{
    ...
    NSDictionary *dic = @{@"companyId":@"",
                          ...
                          @"likeInvoiceCode":@"",
                          @"likeInvoiceTitle":@"",
                          @"searchInvoiceType":self.aarr,
                          @"searchPartnerInfoId":self.arr,
                          };
    NSLog(@"%@",dic);
    NSString *str = [NSString stringWithFormat:@"%@?currentPage=1&pageSize=10&sortName=id&sortType=desc",KURLNSString(@"partner/partnerinvoice/page")];
...
- (void)loddeSLList{
    ...
    NSString *str = [NSString stringWithFormat:@"%@?currentPage=%ld&pageSize=10&sortName=id&sortType=desc",KURLNSString(@"partner/partnerinvoice/page"),page];
    [Manager requestPOSTWithURLStr:str paramDic:dic token:nil finish:^(id responseObject) {
        NSDictionary *diction = [Manager returndictiondata:responseObject];
        if ([Manager judgeWhetherIsEmptyAnyObject:[diction objectForKey:@"rows"]] == YES) {
            NSMutableArray *arr = [diction objectForKey:@"rows"];
