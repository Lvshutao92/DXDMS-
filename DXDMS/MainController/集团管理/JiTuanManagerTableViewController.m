//
//  JiTuanManagerTableViewController.m
//  DXDMS
//
...
@interface JiTuanManagerTableViewController ()
@property(nonatomic,strong)NSMutableArray *arr;
@end

@implementation JiTuanManagerTableViewController
...
- (void)viewDidLoad {
    ...
    self.arr = [@[@"集团列表",@"公司信息"]mutableCopy];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ImgCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        if ([cell.labText.text isEqualToString:@"集团列表"]) {
            JiTuan_jichu_ViewController *jituan = [[JiTuan_jichu_ViewController alloc]init];
            UINavigationController *navi = [[UINavigationController alloc]initWithRootViewController:jituan];
            jituan.navigationItem.title = @"集团列表";
            [self presentViewController:navi animated:YES completion:nil];
        }
        if ([cell.labText.text isEqualToString:@"公司信息"]) {
            JiTuan_gongsi_ViewController *jituan = [[JiTuan_gongsi_ViewController alloc]init];
            UINavigationController *navi = [[UINavigationController alloc]initWithRootViewController:jituan];
            jituan.navigationItem.title = @"公司信息";
            [self presentViewController:navi animated:YES completion:nil];
        }
